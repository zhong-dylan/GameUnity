...
namespace tools
{
    public class ShowAllNotUseResource : EditorWindow
    {
        ...
        #region 编缉器入口
        [MenuItem("Tools/ShowAllNotUseResource", false, 40)]
        public static void Open()
        {
            ShowAllNotUseResource window = EditorWindow.GetWindow<ShowAllNotUseResource>();
            window.titleContent = new GUIContent("ShowAllNotUseResource");
        ...
        private void StartCheck()
        {
            ...
            string[] files = Directory.GetFiles(Application.dataPath + "/Resources", "*.*", SearchOption.AllDirectories)
                .Where(s => withoutExtensions.Contains(Path.GetExtension(s).ToLower())).ToArray();

            List<string> withoutExtensions2 = new List<string>() { ".png",".jpg" };
            //所有project 里面的资源
            string[] UIReource = Directory.GetFiles(Application.dataPath + "/Resources", "*.*", SearchOption.AllDirectories)
